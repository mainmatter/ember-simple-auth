/**
  ...
  defined as `loginRoute` in [Ember.SimpleAuth.setup](#Ember.SimpleAuth_setup) (or `login` by default).
...
Ember.SimpleAuth.AuthenticatedRouteMixin = Ember.Mixin.create({
  /**
    ...
    it redirects to the route defined as `loginRoute` in [Ember.SimpleAuth.setup](#Ember.SimpleAuth_setup) (or `login` by default).
    It also intercepts the current transition so that it can be retried after the user
    has authenticated (see [ApplicationRouteMixin#loginSucceeded](#Ember.SimpleAuth.ApplicationRouteMixin_loginSucceeded)).
