...
test('loads all properties', function() {
  deepEqual(store.loadAll(), {}, 'Ember.SimpleAuth.Stores.Cookie returns an empty plain object when all properties are loaded but the store is empty');

  store.save({ key1: 'value1', key2: 'value2' });
  deepEqual(store.loadAll(), { key1: 'value1', key2: 'value2' }, 'Ember.SimpleAuth.Stores.Cookie loads all stored properties.');
});
