...
test('clears itself', function() {
  ...
  equal(store.load('key1'), null, 'Ember.SimpleAuth.Stores.Ephemeral deletes all properties when cleared.');
  equal(store.load('key2'), null, 'Ember.SimpleAuth.Stores.Ephemeral deletes all properties when cleared.');
});
...
test('saves properties', function() {
  ...
  store.save({ key: '' });
  equal(store.load('key'), null, 'Ember.SimpleAuth.Stores.Ephemeral deletes empty properties when saving.');
});
