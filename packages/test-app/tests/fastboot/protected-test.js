...
module('FastBoot: /protected', function (hooks) {
  ...
  test('can be accessed when the session is authenticated', async function (assert) {
    ...
    let { url, statusCode } = await visit('/protected', {
      headers: {
        cookie:
          'ember_simple_auth-session=%7B%22authenticated%22%3A%7B%22authenticator%22%3A%22authenticator%3Aoauth2%22%2C%22access_token%22%3A%22secret%20token!%22%2C%22account_id%22%3A1%7D%7D',
      },
    });
