...
export default Ember.Service.extend(Ember.Evented, {
  ...
  */
  ...
  /**
    ...
    @param {Any} [...args] Any additional parameters to be passed on to the authorizer.
    ...
    @public
  */
  authorize(authorizerFactory, ...args) {

    // This is the old implementation (deprecated now)
    const oldAuthrize = (authorizerFactory, block) => {
      Ember.deprecate(`Ember Simple Auth: Synchronous authorizers have been deprecated. You shouldn't pass callback function to session.authorize anymore, it returns promise.`, false, {
    ...
    // The new promise-based implementation
    const newAuthorize = (authorizerFactory, ...args) => {
      return new Ember.RSVP.Promise((resolve, reject) => {
        if (this.get('isAuthenticated')) {
          const authorizer = getOwner(this).lookup(authorizerFactory);
          const sessionData = this.get('data.authenticated');
          return this._callAuthorizerAsync(authorizer, sessionData, ...args).then(resolve, reject);
        } else {
          reject();
        }
      });
    ...
    // if the caller passes callback function, use the old implementation
    if (typeof args[0] === 'function') {
      return oldAuthrize(authorizerFactory, args[0]);
    } else {
      return newAuthorize(authorizerFactory, ...args);
    }
  },

  /**
    Call the authorizer and return a promise regardless of whether the authorizer
    is asynchronous or not.

    @param {Object} authorizer The authorizer to authorize the request with.
    @param {Any} [...args] All parameters that the authorizer needs in order to authorize the request.
    @return {Ember.RSVP.Promise} A promise that resolves after authrorization is complete with an object that has two keys: headerName and headerValue.
    @private
  */
  _callAuthorizerAsync(authorizer, ...args) {
    let result;

    // Guard the authorizer call with try/catch block to make sure this function
    // always returns a promise. Even with malfunctioning authorizers.
    try {
      result = authorizer.authorize(...args);
    } catch (e) {
    ...
    if (result && typeof result.then === 'function') {
      ...
      Ember.deprecate(`Ember Simple Auth: Synchronous authorizers have been deprecated. Authorizer.authorize() should return a promise.`, false, {
        id: `ember-simple-auth.authorizer.authorize`,
        until: '2.0.0'
      });
