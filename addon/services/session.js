...
export default Ember.Service.extend(Ember.Evented, {
  ...
  */
  ...
  /**
    ...
    @param {Object} ajaxSetup jQuery ajaxSetup hash with information about the request to be made
    @public
  */
  authorize(authorizerFactory, block, ajaxSetup) {
    if (this.get('isAuthenticated')) {
      const authorizer = getOwner(this).lookup(authorizerFactory);
      const sessionData = this.get('data.authenticated');
      authorizer.authorize(sessionData, block, ajaxSetup);
    }
  }
});
